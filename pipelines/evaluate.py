...
def run_backtest(currency, test_days=30):
    print(f"\n🔄 Menjalankan Backtest {test_days} Hari untuk {currency}...")
    ...
    # 2. Muat Model yang Sudah Dilatih Semalaman
    manager = ModelManager(currency)
    if not manager.load_all_models():
        print(f"❌ Model {currency} tidak ditemukan. Lewati.")
        return

    results = {'ARIMA': [], 'LSTM': [], 'ARIMA-LSTM Hybrid': [], 'Actual': []}

    # 3. Looping Simulasi Prediksi Harian (Walk-Forward)
    for i in range(test_days, 0, -1):
        # Sembunyikan data masa depan dari model (hanya berikan data sampai H-1)
        df_known = df_merged.iloc[:-i][['Close Price']]
        exog_known = df_merged[['Open_lag1', 'High_lag1', 'Low_lag1', 'Close_lag1', 'Return', 'HL_Spread', 'Inflasi', 'BI Rate']].iloc[:-i]
    ...
    # 4. Kalkulasi Metrik (Rumus Skripsi)
    y_true = np.array(results['Actual'])
    print(f"✅ Evaluasi Selesai! Berikut adalah format dictionary untuk UI Streamlit-mu:\n")
    ...
    for model_name in ['ARIMA', 'LSTM', 'ARIMA-LSTM Hybrid']:
        ...
        print(f"        'CI Coverage': 95.0  # Asumsi statis untuk tampilan UI")
        print(f"    }},")
    print("}")
    print("=====================================================")

if __name__ == "__main__":
    # Jalankan untuk ketiga mata uang
